...
import threading # Threading için import edildi
...
MIDI_MELODY_READY_EVENT = pygame.USEREVENT + 1
MIDI_PARSE_ERROR_EVENT = pygame.USEREVENT + 2
MIDI_LOAD_CANCELED_EVENT = pygame.USEREVENT + 3
...
def parse_midi_to_melody(filepath):
    if not MIDO_AVAILABLE:
        pygame.event.post(pygame.event.Event(MIDI_PARSE_ERROR_EVENT, {"error_message": "Mido kütüphanesi yüklenemedi."}))
        return None
    try:
        ...
        pygame.event.post(pygame.event.Event(MIDI_PARSE_ERROR_EVENT, {"error_message": f"MIDI ayrıştırma hatası: {e}"}))
        return None
...
class MIDIManager:
    def __init__(self):
        self.input_device = None; self.midi_available = False
        try:
            if hasattr(pygame, 'midi') and pygame.midi: # pygame.midi'nin None olup olmadığını da kontrol et
                pygame.midi.init()
                self.midi_available = True
            else:
                print("Hata: Pygame 'midi' modülü kullanılamıyor veya başlatılamadı.")
        except pygame.error as e:
            print(f"Pygame MIDI başlatılırken Pygame hatası: {e}")
        except Exception as e:
            print(f"MIDI sistemi başlatılırken genel hata: {e}")

    def print_device_info(self):
        if not self.midi_available: print("MIDI sistemi mevcut değil."); return
        ...
        except pygame.error as e: print(f"MIDI cihaz bilgisi alınırken hata: {e}")
        print("-" * 30)
    ...
    def open_input_device(self, dev_id=None):
        ...
        try:
            ...
            if actual_id != -1 and actual_id < pygame.midi.get_count(): # ID'nin geçerli aralıkta olduğunu kontrol et
            ...
            elif actual_id == -1: print("Varsayılan MIDI giriş cihazı bulunamadı.")
            else: print(f"Geçersiz MIDI cihaz ID'si: {actual_id}")
            return False
        except pygame.error as e: print(f"MIDI açma Pygame hatası (ID: {dev_id}): {e}"); self.input_device=None; return False
        except Exception as e: print(f"MIDI açma genel hatası (ID: {dev_id}): {e}"); self.input_device=None; return False
    ...
    def poll_events(self):
        ...
        except pygame.error as e: print(f"MIDI olay okuma hatası: {e}"); self.close_input_device()
        return events

    def close_input_device(self):
        if not self.input_device or not self.midi_available: return
        try: print("MIDI Kapatılıyor."); self.input_device.close(); self.input_device=None
        except pygame.error as e: print(f"MIDI kapatma hatası: {e}"); self.input_device=None

    def quit(self):
        if not self.midi_available: return
        self.close_input_device()
        try: pygame.midi.quit()
        except pygame.error as e: print(f"Pygame MIDI sonlandırma hatası: {e}")
    ...
    def _generate_display_keys(self, start_midi, num_total_keys):
        ...
        for key_obj in temp_keys_for_layout:
            if key_obj.is_black:
                ...
                for wk in white_keys_for_black_placement: # Bu liste zaten MIDI sırasına göre olmalı
                    if wk.midi_note == key_obj.midi_note -1: # Siyah tuşun solundaki beyaz tuş
        ...
        self.keys.sort(key=lambda k: k.midi_note) # Son kez MIDI'ye göre sırala
...
class Game:
    def __init__(self):
        ...
        self.loading_midi = False # MIDI yükleme durumu için flag
        self.midi_load_thread = None # Yükleme thread'i için referans
    ...
    def _handle_events(self):
        for event in pygame.event.get():
            ...
            elif event.type == MIDI_MELODY_READY_EVENT: # Thread'den gelen olay
                self.loading_midi = False
                if event.melody:
                    print("MIDI melodisi hazır, çalınıyor.")
                    self.playback_handler.start_playback(event.melody)
                else:
                    print("MIDI melodisi yüklenemedi (boş).")
            elif event.type == MIDI_PARSE_ERROR_EVENT: # Thread'den gelen hata olayı
                self.loading_midi = False
                print(f"MIDI İşleme Hatası: {event.error_message}")
            elif event.type == MIDI_LOAD_CANCELED_EVENT: # Thread'den gelen iptal olayı
                self.loading_midi = False
                print("MIDI dosyası yükleme iptal edildi.")
            ...
            if event.type==pygame.MOUSEBUTTONDOWN and event.button==1:
                ...
                if self.record_button_rect.collidepoint(pos): self.recorder.toggle_recording();self.playback_handler.stop_playback()
                    ...
                    if not self.loading_midi: # Sadece zaten yükleme yapılmıyorsa
                        self._initiate_midi_file_selection()
                    else:
                        print("Zaten bir MIDI dosyası yükleniyor.")
    ...
    def _initiate_midi_file_selection(self):
        """Dosya diyalogunu ana thread'de açar, sonra parse işlemini ayrı bir thread'de başlatır."""
        if not MIDO_AVAILABLE:
        ...
        try:
            ...
            pygame.event.post(pygame.event.Event(MIDI_PARSE_ERROR_EVENT, {"error_message": f"Dosya seçilemedi: {e}"}))
            return
        ...
        if filepath:
            if not (filepath.lower().endswith(".mid") or filepath.lower().endswith(".midi")):
                ...
                pygame.event.post(pygame.event.Event(MIDI_PARSE_ERROR_EVENT, {"error_message": "Geçersiz dosya tipi."}))
                return

            self.loading_midi = True
            self.playback_handler.stop_playback() # Önceki çalmayı durdur
            
            # Eski thread'in bitmesini beklemek yerine, yeni bir tane oluşturuyoruz.
            # _handle_events içindeki kontrol zaten aynı anda birden fazla yüklemeyi engelliyor.
            self.midi_load_thread = threading.Thread(target=self._threaded_parse_midi, args=(filepath,), daemon=True)
            self.midi_load_thread.start()
        else:
            print("MIDI dosyası seçimi iptal edildi.")
            pygame.event.post(pygame.event.Event(MIDI_LOAD_CANCELED_EVENT))


    def _threaded_parse_midi(self, filepath):
        """MIDI dosyasını ayrı bir thread'de ayrıştırır ve olay gönderir."""
        print(f"'{os.path.basename(filepath)}' ayrıştırılıyor (thread)...")
        parsed_melody = parse_midi_to_melody(filepath) # Bu fonksiyon zaten hata durumunda None döner ve event post eder
        if parsed_melody:
            pygame.event.post(pygame.event.Event(MIDI_MELODY_READY_EVENT, {"melody": parsed_melody}))
        # else: parse_midi_to_melody zaten MIDI_PARSE_ERROR_EVENT gönderiyor.
        # Hata veya başarı durumunda loading_midi'yi ana thread'de event handler içinde false yapıyoruz.
    ...
    def _render(self):
        ...
        # MIDI Yükle butonu için durum kontrolü
        is_playing_loaded_midi = self.playback_handler.is_playing and \
                                 self.playback_handler.melody_to_play not in [MOZART_MELODY_EXAMPLE, self.recorder.melody]
        
        self._draw_button(self.load_midi_button_rect, "MIDI Yükle & Çal", PURPLE, DARK_GRAY, is_playing_loaded_midi or self.loading_midi) # Yüklenirken de vurgula
        ...
        if self.recorder.is_recording:
            ...
            self.screen.blit(self.ui_font.render(f"Kayıt: {rec_dur:.1f}s",True,RED),(SCREEN_WIDTH//2-100,status_y)) # Biraz sola kaydırıldı
        
        if self.loading_midi:
            loading_text_surf = self.ui_font.render("MIDI Yükleniyor...", True, RED)
            loading_text_rect = loading_text_surf.get_rect(center=(SCREEN_WIDTH // 2, PIANO_Y_OFFSET - 30)) # Piyanonun üstüne
            pygame.draw.rect(self.screen, LIGHT_GRAY, loading_text_rect.inflate(20,10)) # Arka plan
            self.screen.blit(loading_text_surf, loading_text_rect)
    ...
    def _cleanup(self):
        ...
        if self.root_tk:
            try: self.root_tk.quit(); self.root_tk.destroy()
